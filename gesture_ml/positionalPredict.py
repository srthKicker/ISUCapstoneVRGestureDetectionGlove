...
# ── Config ───────────────────────────────────────────────
SERIAL_PORT    = "/dev/ttyACM0"     # to ttyACM0 your port
BAUD_RATE      = 115200
HAND           = "left"    # "left" or "right"

WINDOW_SIZE    = 10        # MUST match training TIMESTEPS (was 30 — that caused the stuck bug)
...
QUAT_SCALING_FACTOR = 16384.0 # something about int16 scaling
...
def preprocess(window_deque, hand):  # No scaler
    X = np.array(window_deque, dtype=np.float32)
    if hand == "right":
        X = mirror_left_hand(X)  # Only mirroring for right hand
...
def parse_line(line):
    ...
    try:
        ...
        if len(parts) < 24:
            return None
        values = list(map(float, parts[:24]))  # int16 → float
        values = [v / QUAT_SCALING_FACTOR for v in values] # unscale int16 → [-1,1]
...
def load_encoder(hand):
    ...
    """
    for path in [f"gesture_model_{hand}_encoder.pkl", "label_encoder_left.pkl"]:
    ...
    _, _, enc = load_dataset(hand=HAND)
    return enc
...
# Infer WINDOW_SIZE from the model's input shape — no data loading needed
WINDOW_SIZE = model.input_shape[1]
print(f"Window size from model: {WINDOW_SIZE}")
# ── Run inference ─────────────────────────────────────────
